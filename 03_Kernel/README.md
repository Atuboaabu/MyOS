...
## 七、中断(Interrupt)
中断时计算机体系结构中的一种关键机制，允许CPU在运行程序的过程中能够及时响应事件或异常，暂停当前正在执行的程序，切换到特定的中断处理逻辑。  

### 1、中断分类
1. 外部中断：外部中断是指来自CPU外部的中断，而外部中断来源必须为某个硬件，因此外部中断也称为硬件中断。如网卡、键盘等。  
接收外部中断需要CPU有引脚，但是外设可以无限多，但是CPU的引脚是不可能无限多的；CPU接收中断的方案是对外提供统一的接口作为中断信号的公共线路，所有来自外设的信号都通过公共接口连接到CPU。  
CPU提供了两个信号线：INTR(Interupt) 和 NMI(Non Maskable Interupt)。外部中断就是通过这个两个信号线通知CPU的。  
![外部中断接口](../00_image/external_interrupt.png)  
- INTR：可屏蔽中断的引脚；非致命情况的中断都走该引脚连接CPU。  
- NOI: 不可屏蔽中断的引脚，表示系统发生致命错误，会导致CPU宕机等异常情况。如内存访问错误等。  
2. 内部中断：内部中断分为软中断和异常。软中断是由软件主动发起的中断；异常是指令执行过程中CPU产生的错误引起的中断。  
![异常与中断](../00_image/interuptions.png)  

### 2、中断描述符表(IDT: Interupt Descriptor Table)
中断描述符表 IDT 是在保护模式下用于存储中断程序入口的表，当CPU接收到一个中断时，需要用中断向量在此表中检索对应的描述符，在该描述符中找到中断程序的其实地址，然后执行中断处理程序。  
在实模式下，用于存储中断处理程序的入口的表称为中断向量表（IVT: Interupt Vector Table）。  
IDT 中几种门的描述符格式：  
- 任务门：任务门和任务状态段(TSS)是Intel处理器在硬件一级提供的任务切换机制，任务门需要配合TSS一起使用，在任务门中记录TSS选择子。任务门可安装在GDT、LDT和IDT中。   
![任务门](../00_image/task_gate.png)  
- 中断门：中断门包含中断处理程序所在段的选择子和段内偏移地址。当通过此方式进入中断后，标志寄存器eflags中的IF位自动置0，把中断关闭，避免中断嵌套。中断门只允许存在于IDT中。    
![中断门](../00_image/interrupt_gate.png)  
- 陷阱门：陷阱门与中断门类似，区别在于陷阱门进入中断后，不会置位eflags寄存器中的IF位为0。陷阱门也只允许存在于IDT中。    
![陷阱门](../00_image/trap_gate.png)  
- 调用门：调用门是提供给用户进程进入特权0级的方式，其DPL为3。调用门中记录例程的地址，不能使用int指令调用，只能使用jmp和call指令。调用门可安装在GDT和LDT中。    
![调用门](../00_image/call_gate.png)  

中断描述符表的加载：  
对比中断向量表（IVT），中断描述符表（IDT）有两点区别：  
1. 中断描述符表的地址不限制，在哪里都可以；  
2. 中断描述符表中的每个描述符都用8字节描述。  
在CPU内部有一个中断描述符表寄存器IDTR(Interupt Descriptor Table Register)，该寄存器分为两部分：0-15位表示表的界限；16-47位表示IDT的基址。因此IDT的最大范围为0xFFFF，即64KB，可容纳描述符个数为64KB / 8 = 8192个。  
加载IDT的指令位 lidt : lidt 16位界限 32位基址  

### 3、中断处理过程
从中断发生到中断处理这个过程，分为CPU外和CPU内两部分：  
- CPU外：外部设备产生的中断由中断代理芯片接收，处理后将该中断的中断向量号(Vector NO.)发送给CPU。  
- CPU内：CPU中执行中断向量号对应的中断处理程序。  
整个处理过程如下：  
    (1)、处理器根据中断向量号定位中断门描述符。  
    中断向量号是中断描述符的索引，处理器收到中断向量号后，在中断描述符表中查询对应的中断描述符，然后再去执行该中断描述符中的中断处理程序。  
    (2)、处理器进行特权级检查。  
    对于软件主动发起的软中断，当前特权级CPL必须在门描述符DPL和门中目标代码段DPL之间。防止位于3级特权下的用户程序主动调用某些只为内核服务的例程。  
    (3)、执行中断程序。  
    特权级检查通过后，将们描述符目标代码段选择子加载到代码段寄存器CS中，把门描述符中断处理程序的偏移地址加载到EIP中，开始执行中断处理程序。  

### 4、中断发生时的压栈
在32位保护模式下，中断发生时，处理器收到一个中断向量号，根据中断向量号找到对应的中断门描述符，处理器根据描述符加载目标代码段选择子到代码段寄存器CS及偏移量到指令寄存器EIP。此时处理器认为已经换了一个段，属于段间转移。为了执行完中段程序后还能回到该进程继续执行，处理器需要将一些信息压入栈中：  
寄存器入栈情况及顺序如下：  
1. 根据CPL和目标代码段的DPL对比，盘对岸是否涉及特权级的切换，若有特权级的切换，需要将被中断进程的栈SS和ESP压入栈中。  
2. 将标志寄存器eflags压入栈中。  
3. 由于切换到目标代码段，需要将CS和EIP压入栈中备份。  
4. 某些异常会存在错误码，错误码用于报告异常发生的段及选择子等信息。因此错误码会在EIP之后入栈。  
综上，涉及到的压入栈中的寄存器包含：SS、ESP、EFLAGS、CS、EIP、ERRORCODE。详细信息如下图：  
![中断发生时入栈情况](../00_image/interupt_push_stack.png)  

执行完中断处理程序后，还要返回到被中断的进程，中段返回通过 iret 指令实现，此时将会从栈中依次弹出32位数据分别到寄存器EIP、CS、EFLAGS。根据特权级是否有变化还会有ESP、CS寄存器。  

**中断错误码**   
中断错误码格式如下图所示：  
![中断错误码](../00_image/interupt_errcode.png)  
- EXT: 标识事件是否来自外部。  
- IDT: 标识该选择子是否指向中断描述符IDT，IDT位为1，则表示此描述符指向中断描述符表，否则就是指向GDT或LDT。  
- TI: 当IDT为0，TI才有效，TI为1表示LDT，为1表示GDT。  
